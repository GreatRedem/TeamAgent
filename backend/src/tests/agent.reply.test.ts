...
function main() {
    ...
    const tests: Array<[string, () => void]> = [
        ...
        [
            ...
            () => {
                ...
                const prompt = buildSystemPrompt(
                    [...docs, { name: 'manual.md', content: big }],
                    true,
                );
                ...
                assert.ok(
                    prompt.includes('document_read'),
                    'nothing told the model how to open it',
                );
        ...
        [
            ...
            () => {
                for (const window of [512, 1024, 2048, 4096, 8192, 200_000]) {
                    assert.ok(
                        completionCap(window) <= Math.max(256, window / 2),
                        `window ${window}`,
                    );
                    assert.ok(
        ...
        [
            ...
            () => {
                assert.deepEqual(
                    readUsage({ usage: { prompt_tokens: 1200, completion_tokens: 85 } }),
                    {
                        prompt: 1200,
                        completion: 85,
                    },
                ...
                assert.deepEqual(
                    readUsage({ usage: { prompt_tokens: -3, completion_tokens: 'x' } }),
                    {
                        prompt: 0,
                        completion: 0,
                    },
                ...
                assert.deepEqual(
                    readUsage({
                        usage: { promptTokens: 310, completionTokens: 22, totalTokens: 332 },
                    }),
        ...
        [
            ...
            () => {
                ...
                assert.ok(
                    countTokens(call, sent, true).completion > 0,
                    'a tool call counts as output',
                );
        ...
        [
            ...
            () => {
                const text = readCompletion({
                    choices: [{ message: { content: 'x'.repeat(9000) } }],
                });
        ...
        [
            ...
            () => {
                ...
                assert.equal(
                    readError({ error: { message: 'x'.repeat(500) } }).length,
                    ERROR_TEXT_MAX,
                );
            },
        ],

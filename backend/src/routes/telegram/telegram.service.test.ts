...
const pm = (overrides: Record<string, unknown> = { }, from: Record<string, unknown> = { }) => ({
    ...
    message: {
        message_id: 42,
        from: { id: 7123456789012345, is_bot: false, first_name: 'Sara', last_name: 'K', username: 'sara_k', language_code: 'fa', ...from },
...
const tests: Array<[ string, () => void ]> = [
    ...
    {
        assert.equal(readInboundMessage(pm({ chat: { id: -100123, type: 'group', title: 'Team' } })), undefined);
    } ],

    [ 'a supergroup message is ignored', () =>
    {
        assert.equal(readInboundMessage(pm({ chat: { id: -100123, type: 'supergroup', title: 'Team' } })), undefined);
    } ],
    ...
    {
        assert.equal(readInboundMessage({ update_id: 1, edited_message: { text: 'x' } }), undefined);
    } ],
    ...
    [ 'a body that is not an update is ignored', () =>
    {
        ...
        {
            assert.equal(readInboundMessage(body), undefined, `accepted ${ JSON.stringify(body) }`);
        }
    } ],

    [ 'missing name fields become empty strings, not undefined', () =>
    {
        const inbound = readInboundMessage(pm({ }, { username: undefined, last_name: undefined, language_code: undefined }));
    ...
    {
        const inbound = readInboundMessage(pm({ text: 'x'.repeat(20000) }, { first_name: 'A'.repeat(5000), username: 'b'.repeat(500) }));
    ...
    [ 'a missing date falls back to now rather than the epoch', () =>
    {
        ...
        assert.ok(inbound);
        assert.ok(Date.now() - inbound.sentAt.getTime() < 5000, `got ${ inbound.sentAt.toISOString() }`);
    } ]
];

...
import {
    MEMBERS_MAX, ROSTER_CONTENT_MAX, RosterError, emptyRoster, findMember,
    parseRoster, removeMember, serializeRoster, upsertMember } from './team.roster.js';

const sample = serializeRoster({
    members: [
        { name: 'Alex', rank: 'founder', description: 'runs the place', social: { x: '@alex', github: 'alexk' } },
...
const tests: Array<[ string, () => void ]> = [
    ...
    [ 'a damaged file is refused, not read as empty', () =>
    {
        ...
        assert.throws(() => parseRoster('{"members":{}}'), RosterError, 'accepted a non-array members');
    } ],

    [ 'a member with no usable name is dropped, not kept as a blank', () =>
    {
        const roster = parseRoster(JSON.stringify({ members: [
            { name: 'Real' }, { name: '   ' }, { rank: 'nameless' }, null, 'string', [ ]
        ] }));

        assert.deepEqual(roster.members.map((member) => member.name), [ 'Real' ]);
    } ],
    ...
    {
        const roster = upsertMember(parseRoster(sample), { name: 'Alex', social: { linkedin: 'alexk' } });

        assert.deepEqual(findMember(roster, 'Alex')?.social, { x: '@alex', github: 'alexk', linkedin: 'alexk' });
    } ],
    ...
    {
        const members = Array.from({ length: 200 }, (_, at) => ({ name: `member-${ at }`, description: 'x'.repeat(2000) }));
    ...
    {
        const members = Array.from({ length: MEMBERS_MAX }, (_, at) => ({ name: `member-${ at }` }));
    ...
    {
        const roster = parseRoster(JSON.stringify({ members: [ { name: 'Alex', social: { x: '@alex', broken: 42 } } ] }));

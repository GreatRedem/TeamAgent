...
const tests: Array<[string, () => Promise<void>]> = [
    [
        'a live bot reports its username',
        async () => {
            await withFetch(
    ...
    [
        'an unknown token (404) is rejected',
        async () => {
    ...
    [
        'a 200 carrying ok:false is rejected',
        async () => {
    ...
    [
        'a network failure is unreachable, not rejected',
        async () => {
            await withFetch(
                () => Promise.reject(new Error('getaddrinfo ENOTFOUND api.telegram.org')),
                async () => {
                    assert.deepEqual(await probeTelegram(TOKEN), {
                        ok: false,
    ...
    [
        'no outcome carries the token back to the caller',
        async () => {
            const stubs = [
                json(200, { ok: true, result: { username: 'support_bot' } }),
                ...
                () =>
                    Promise.reject(
                        new Error(`failed to reach https://api.telegram.org/bot${TOKEN}/getMe`),
                    ),
            ];
            ...
            for (const stub of stubs) {
                await withFetch(stub, async () => {
                    ...
                    assert.equal(
                        JSON.stringify(probe).includes(TOKEN),
                        false,
                        `token leaked: ${JSON.stringify(probe)}`,
                    );
                });
            }
        },

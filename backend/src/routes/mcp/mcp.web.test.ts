...
const ALLOWED = [ '1.1.1.1', '8.8.8.8', '93.184.216.34', '172.32.0.1', '172.15.0.1', '192.167.1.1', '2606:4700:4700::1111' ];
...
const tests: Array<[ string, () => void | Promise<void> ]> = [
    ...
    [ 'non-http schemes are refused', async() =>
    {
    ...
    [ 'malformed urls are refused', async() =>
    {
    ...
    [ 'literal internal addresses are refused without needing DNS', async() =>
    {
        for (const url of [ 'http://127.0.0.1/', 'http://169.254.169.254/latest/meta-data/', 'http://10.0.0.5:8080/admin', 'http://[::1]:1000/' ])
        {
            const result = await checkPublicUrl(url);
    ...
    [ 'a public hostname is allowed', async() =>
    {

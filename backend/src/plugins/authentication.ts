...
export function verifyAccessToken(token: string) {
    ...
    if (
        signatureServer.length !== signature.length ||
        !timingSafeEqual(signatureServer, signature)
    ) {
        return;
    }
...
export function createAccessToken(id: number, role: number, sessionId: number): string {
    const payload = Buffer.from(
        JSON.stringify({
            ...
            role,
            sid: sessionId,
            expires_at: Math.floor((Date.now() + SESSION_ACCESS_TIME) / 1000),
        }),
    ).toString('base64url');
...
export function createRefreshToken(id: number, role: number): string {
    const payload = Buffer.from(
        JSON.stringify({
            id,
            role,
            expires_at: Math.floor((Date.now() + SESSION_REFRESH_TIME) / 1000),
        }),
    ).toString('base64url');
...
export default fastifyPlugin(async (fastify) => {
    ...
    fastify.addHook('preHandler', async (request: FastifyRequest, reply: FastifyReply) => {
        ...
        request.log.warn(
            { module: 'auth', url: request.url, required: routeRole, actual: request.account_role },
            'forbidden request',
        );

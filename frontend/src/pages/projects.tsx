...
export function Projects()
{
    ...
    useEffect(() =>
    {
        ...
        teamList()
            ...
            .catch((cause: unknown) =>
            {
                ...
                setTeams([ ]);
                setError(cause instanceof ApiError ? cause.result : 'Your projects could not be loaded.');
            });
    ...
    const create = useCallback(async(event: React.FormEvent) =>
    {
        ...
        {
            setFormError(cause instanceof ApiError ? cause.result : 'The project could not be created.');
        }
        finally
        {
            setBusy(false);
        ...
        <>
                ...
                <DialogContent>
                    <form className="grid gap-5" onSubmit={ create }>
                        <DialogHeader>
                            ...
                            <DialogDescription>Group the bots, agents and models that belong together.</DialogDescription>
                        </DialogHeader>
            ...
            { teams === null && (
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    { [ 0, 1, 2 ].map((i) => <Skeleton className="h-40 rounded-xl" key={ i } />) }
                </div>
            ) }
                            ...
                            <Card className="h-full gap-3 transition-colors hover:border-input">
                                ...
                                <CardContent>
                                    <p className="m-0 line-clamp-2 min-h-10 text-sm text-muted-foreground">
                                        { team.description === '' ? 'No description yet.' : team.description }
                                    </p>
                                </CardContent>

                                <CardFooter className="justify-between gap-3">
                                    <time className="font-mono text-2xs text-muted-foreground" dateTime={ team.created_at }>
            ...
            { page !== null && teams !== null && teams.length > 0 && (
                <PaginationFooter page={ page } shown={ teams.length } busy={ paging } noun="projects" onPage={ (offset) => void goTo(offset) } />
            ) }
        </>
    );
}

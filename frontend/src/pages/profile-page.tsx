...
export function ProfilePage()
{
    ...
    useEffect(() =>
    {
        ...
        Promise.all([ profileDetails(teamId, personId), profileFiles(teamId, personId), permissionCatalog(teamId) ])
            ...
            .catch((cause: unknown) =>
            {
                ...
                setError(cause instanceof ApiError ? cause.result : 'This person could not be loaded.');
            });
    ...
    const goToFiles = useCallback(async(offset: number) =>
    {
        ...
        {
            setError(cause instanceof ApiError ? cause.result : 'The notes could not be loaded.');
        }
        finally
        {
            setPaging(false);
    ...
    const toggle = useCallback(async(key: string) =>
    {
        ...
        {
            setError(cause instanceof ApiError ? cause.result : 'The permission could not be changed.');
        }
        finally
        {
            setSaving(null);
    ...
    if (idsInvalid)
    {
        return <Alert variant="destructive"><AlertDescription>That person address is not valid.</AlertDescription></Alert>;
    }
    ...
    return (
        <>
            <PageHeader
                ...
                description={ details === null ? 'Loading this person.' : `${ details.profile.message_count.toLocaleString() } message${ details.profile.message_count === 1 ? '' : 's' } across ${ details.bots.length } bot${ details.bots.length === 1 ? '' : 's' }.` }
                actions={ (
                    ...
                    <div className="grid gap-6">
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>The newest messages Nura has stored for this person.</CardDescription>
                            </CardHeader>

                            <CardContent className="grid gap-3">
                                { details.messages.length === 0 && (
                                    <p className="m-0 text-sm text-muted-foreground">No messages stored yet.</p>
                                ) }
                        ...
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Files an agent has written about this person.</CardDescription>
                            </CardHeader>

                            <CardContent className="grid gap-3">
                                { files.length === 0 && (
                                    <p className="m-0 text-sm text-muted-foreground">No agent has written anything about this person yet.</p>
                                ) }

                                { files.map((file) => (
                                    <div className="grid gap-2 rounded-lg border p-4" key={ file.id }>
                                        <div className="flex flex-wrap items-baseline justify-between gap-2">
                                            <p className="m-0 font-mono text-sm font-semibold">{ file.name }</p>
                                            <p className="m-0 font-mono text-2xs text-muted-foreground">{ tokenLabel(file.content) }</p>
                                        </div>

                                        <pre className="m-0 max-h-60 overflow-auto rounded-md border bg-well p-3 font-mono text-2xs whitespace-pre-wrap">{ file.content }</pre>
                                    </div>
                                )) }
                            </CardContent>
                            ...
                            { filePage !== null && files.length > 0 && (
                                <CardFooter className="border-t">
                                    <PaginationFooter page={ filePage } shown={ files.length } busy={ paging } noun="notes" onPage={ (offset) => void goToFiles(offset) } />
                                </CardFooter>
                            ) }
                        </Card>
                    </div>
                    ...
                    <div className="grid gap-6 xl:sticky xl:top-32">
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>What Telegram reports about this person.</CardDescription>
                            </CardHeader>

                            <CardContent>
                                <dl className="m-0 grid grid-cols-[auto_1fr] items-baseline gap-x-4 gap-y-2 text-sm">
                                    <Detail label="Telegram id" value={ details.profile.telegram_id } />
                                    <Detail label="Username" value={ details.profile.username === '' ? '' : `@${ details.profile.username }` } />
                                    <Detail label="Language" value={ details.profile.language_code } />
                                    <Detail label="First seen" value={ new Date(details.profile.created_at).toLocaleDateString() } />
                                    <Detail label="Last seen" value={ new Date(details.profile.last_seen_at).toLocaleString() } />
                                </dl>

                                { details.bots.length > 0 && (
                                    <ul className="m-0 mt-5 grid list-none gap-2 border-t p-0 pt-4">
                                        { details.bots.map((bot) => (
                                            <li className="flex items-baseline justify-between gap-3 text-sm" key={ bot.id }>

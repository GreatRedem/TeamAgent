...
{
    return (
        <svg className="absolute inset-x-0 top-0 h-[72%] w-full opacity-55 lg:opacity-100" viewBox="0 0 1440 640" preserveAspectRatio="xMidYMid slice" role="presentation" focusable="false">
            ...
            <g fill="var(--glow-bright)">
                { LATTICE_NODES.map((node, i) => (
                    <circle key={ `n-${ i }` } cx={ node.x } cy={ node.y } r={ node.r } opacity={ node.opacity } />
                )) }
            </g>
        </svg>
    );

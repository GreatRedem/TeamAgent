...
export function DocumentEditor({ teamId, agentId, document, onSaved, onRemoved }: {
    teamId: number;
    agentId: number;
        ...
        <Card className="gap-4">
            <CardHeader>
                ...
                <div className="col-start-2 row-span-2 row-start-1 flex items-center gap-2 self-start justify-self-end">
                    ...
                    <ConfirmButton label="Delete" confirmLabel="Delete for good" onConfirm={ () => void remove() } />
                </div>
            </CardHeader>
            ...
            <CardContent className="grid gap-2">
                ...
                <p className="m-0 text-2xs text-muted-foreground">
                    { tokenLabel(content) }
                    { always ? ', sent with every message this agent answers.' : ', charged only when the agent opens this file.' }
                </p>

    ...
    return (
        <div className="grid gap-3">
            <div className="flex items-start gap-2 overflow-x-auto pb-1">
                <div className="flex shrink-0 flex-col gap-1 text-2xs text-muted-foreground" aria-hidden="true">
                    { WEEKDAYS.map((label, i) => (
                        <span className="flex h-3 items-center leading-none" key={ label }>{ i % 2 === 1 ? label : '' }</span>
                    )) }
                </div>
                    ...
                    { weeks.map((week, w) => (
                        <div className="flex flex-col gap-1" key={ w }>
                            { week.map((day, d) => day === null
                                ...
                                : (
                                    <span
                                        className={ cn('size-3 rounded-sm', HEAT_SCALE[level(day, busiest)], day.errors > 0 && 'ring-1 ring-destructive') }
                                        key={ d }
            ...
            <p className="m-0 flex flex-wrap items-center gap-2 text-2xs text-muted-foreground">
                <span>Quieter</span>
                { HEAT_SCALE.map((fill) => <span className={ cn('size-3 rounded-sm', fill) } key={ fill } />) }
                <span>Busier</span>

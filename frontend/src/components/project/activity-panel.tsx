...
import { ApiError, auditHeatmap, auditList, type AuditEntry, type HeatmapDay, type Paged } from '@/api';
    ...
    useEffect(() =>
    {
        ...
        Promise.all([ auditHeatmap(teamId), auditList(teamId) ])
            ...
            .catch((cause: unknown) =>
            {
                ...
                {
                    setEntries([ ]);
                    setError(cause instanceof ApiError ? cause.result : 'The activity trail could not be loaded.');
                }
            });
    ...
    return (
        <div className="grid gap-6">
            <Card>
                ...
                <CardContent>
                    { heatmap === null ? <Skeleton className="h-28 rounded-lg" /> : <Heatmap days={ heatmap.days } busiest={ heatmap.busiest } /> }
                </CardContent>
            </Card>
            ...
            <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_21rem]">
                <Card className="overflow-hidden">
                    <CardHeader>
                        ...
                        <CardDescription>What was sent, what came back, and how long it took.</CardDescription>
                        ...
                        <div className="col-start-2 row-span-2 row-start-1 flex gap-1 self-start justify-self-end rounded-md bg-muted p-1">
                            ...
                            <Button
                                ...
                                aria-pressed={ failuresOnly }
                                className={ failuresOnly ? 'text-destructive' : 'text-muted-foreground' }
                    ...
                    <CardContent className="px-0">
                        ...
                        { entries === null && error === null && (
                            <div className="grid gap-2 px-5">
                                { [ 0, 1, 2, 3, 4 ].map((i) => <Skeleton className="h-10" key={ i } />) }
                            </div>
                        ) }
                                    ...
                                    { rows.map((entry) =>
                                    {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <TableCell className="max-w-[10rem] truncate">{ row !== null && row.agent !== '' ? row.agent : entry.actor }</TableCell>
                                                ...
                                                <TableCell className="hidden max-w-[12rem] truncate font-mono text-2xs text-muted-foreground lg:table-cell">
                                                    { row !== null && row.messages !== '' ? `${ Number(row.messages).toLocaleString() } in / ${ Number(row.chars).toLocaleString() } chars` : entry.target }
                                                </TableCell>
                                                <TableCell className={ cn('text-right font-mono text-2xs', entry.outcome === 'error' ? 'text-destructive' : 'text-muted-foreground') }>
                                                    { entry.duration_ms > 0 ? entry.duration_ms.toLocaleString() : '—' }
                                                </TableCell>
                                                <TableCell className={ cn('pe-5 text-right font-medium', result.className) }>{ result.label }</TableCell>
                                            </TableRow>
                                        );
                                    }) }
                                </TableBody>
                ...
                <Card className="xl:sticky xl:top-32">
                    <CardHeader>
                        <CardTitle>{ selected === null ? 'Nothing selected' : trip === null ? selected.action : 'Model round-trip' }</CardTitle>
                        <CardDescription>
                            { selected === null
                    ...
                    <CardContent className="grid gap-5">
                        { selected !== null && (
                            <>
                                <DataList>
                                    { trip !== null && trip.agent !== '' && <DataRow label="Agent">{ trip.agent }</DataRow> }
                                    { trip !== null && trip.model !== '' && <DataRow label="Model">{ trip.model }</DataRow> }
                                    { trip !== null && trip.messages !== '' && <DataRow label="Messages in">{ Number(trip.messages).toLocaleString() }</DataRow> }
                                    { trip === null && <DataRow label="Action">{ selected.action }</DataRow> }
                                    { trip === null && selected.target !== '' && <DataRow label="Target">{ selected.target }</DataRow> }
                                    <DataRow label="Actor">{ selected.actor }</DataRow>
                                    { selected.duration_ms > 0 && <DataRow label="Took">{ `${ selected.duration_ms.toLocaleString() } ms` }</DataRow> }
                                </DataList>

                                <div className="grid gap-2">
                                    <p className="m-0 text-sm text-muted-foreground">{ trip === null ? 'Detail' : 'Came back' }</p>

                                    <p className={ cn(
                                        'm-0 rounded-md border bg-well p-3 font-mono text-2xs leading-relaxed break-anywhere',
                                        selected.outcome === 'error' ? 'border-destructive/40 text-destructive' : 'text-muted-foreground'
                                    ) }>
                                        { trip !== null && selected.outcome === 'ok' && trip.chars !== ''
                                            ...
                                            : selected.detail !== '' ? selected.detail : 'Nothing was recorded.' }
                                    </p>
                                </div>
                            </>
                        ) }

...
export function WalletSignIn() {
    ...
    const signIn = useCallback(async () => {
        ...
        if (!provider) {
            setStatus({
                kind: 'error',
                message: 'No wallet found in this browser. Install one, then try again.',
            });
        ...
        try {
            ...
            if (!address) {
                setStatus({
                    kind: 'error',
                    message: 'Your wallet did not share an account. Unlock it and try again.',
                });
            ...
            if (typeof signed !== 'string') {
                setStatus({
                    kind: 'error',
                    message: 'Your wallet returned a signature Nura could not read.',
                });

    ...
    return (
        <DropdownMenu open={ open } onOpenChange={ setOpen }>
            ...
            <DropdownMenuContent align="start" className="w-60">
                ...
                { teams === null && <p className="m-0 px-2 py-1.5 text-sm text-muted-foreground">Loading…</p> }

                { teams !== null && teams.length === 0 && (
                    <p className="m-0 px-2 py-1.5 text-sm text-muted-foreground">No projects yet.</p>
                ) }

                { teams?.map((team) => (
                    <DropdownMenuItem key={ team.id } asChild>
                        <Link to={ teamPath(team.id) }>
                            <Check className={ team.id === teamId ? 'opacity-100' : 'opacity-0' } aria-hidden="true" />
